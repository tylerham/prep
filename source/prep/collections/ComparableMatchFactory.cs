...
namespace prep.collections
{
    ...
    where TProperty : IComparable<TProperty>
  {
    ...
    public ComparableMatchFactory(PropertyAccessor<ItemToFilter, TProperty> accessor,
                                  ICreateMatchers<ItemToFilter, TProperty> original)
    {
    ...
    {
      return new ConditionalMatch<ItemToFilter>(x => accessor(x).CompareTo(value) > 0);
    }

    public IMatchAn<ItemToFilter> between(TProperty start, TProperty end)
    {
      return
        new ConditionalMatch<ItemToFilter>(x => accessor(x).CompareTo(start) >= 0 && accessor(x).CompareTo(end) <= 0);
    }
  }
}
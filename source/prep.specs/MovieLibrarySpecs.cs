...
namespace prep.specs
{
  ...
  public class MovieLibrarySpecs
  {
    ...
    public class when_searching_for_movies : concern_for_searching_and_sorting
    {
      ...
      {
        // > 2004
        var criteria = Where<Movie>.has_an(x => x.date_published.Year).falls_in(new Range<int>().at_least(2004));
      ...
      {
        //1982-2003 - inclusive
        var criteria = Where<Movie>.has_an(x => x.date_published.Year).falls_in(new Range<int>().at_least(1982).at_most(2003));
